...
class InGame {
    ...
    func spawnMinionOn30sec() {
        minionSpawnTimer = Timer.scheduledTimer(timeInterval: 30, target: self, selector: #selector(spawnMinion), userInfo: nil, repeats: true)
        print(minionAlive)
    }
...
//    }
    ...
    func champAttackMinion() {
        ...
        // 미니언 처치 시 골드 획득 한게 골드 획득 라벨에 반영 될 수 있도록 해야함
        self.delegate?.showGold()
        self.delegate?.showMinionCount()
    }
    ...
    // 이걸로 마지막 뷰컨 갈 수 있는거 아님? 왜 못감? 메서드가 실행은 되는데 안 넘어가네
